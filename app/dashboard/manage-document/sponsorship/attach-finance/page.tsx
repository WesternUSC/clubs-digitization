...
export default function AttachInvoiceToPOPage() {
    ...
    async function handleLoadSponsor(e: React.FormEvent) {
        e.preventDefault();
        if (!logId) return;
    ...
    async function handleAttachFinance(e: React.FormEvent) {
        e.preventDefault();
        if (!financeFile || !originalLogId) return;

        setIsSubmitting(true);
        try {
            const formData = new FormData();
            formData.append("financeFile", financeFile);
            ...
            const res = await fetch("/api/attach-finance", { method: "POST", body: formData });
            if (res.ok) {
                const data = await res.json();
        ...
        <div className="space-y-6">
            ...
            <Breadcrumb className="mb-6">
                <BreadcrumbList>
                    ...
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                        <BreadcrumbPage>Attach Finance to Sponsorship</BreadcrumbPage>
                    </BreadcrumbItem>
                </BreadcrumbList>
            </Breadcrumb>
            ...
            {/* Page Header */}
            <div>
                <h1 className="text-3xl font-bold tracking-tight">Attach Finance to Sponsorship</h1>
            ...
            <Card>
                ...
                <CardContent>
                    <form onSubmit={handleLoadSponsor} className="space-y-4">
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="space-y-6">
                            ...
                            <form onSubmit={handleAttachFinance} className="space-y-6 pt-4 border-t">
                                <div className="space-y-2">
                                    <Label htmlFor="contract-upload">Upload Finance</Label>
                                    <Input
                                        id="finance-upload"
                                        type="file"
                                        ...
                                        required
                                    />
                                    {financeFile && <p className="text-sm text-muted-foreground">Selected file: {financeFile.name}</p>}
                                </div>
                                <Button type="submit" className="w-full" disabled={!financeFile || isSubmitting}>
                                    {isSubmitting ? (
                                        <>
                                            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                            Attaching Finance...
