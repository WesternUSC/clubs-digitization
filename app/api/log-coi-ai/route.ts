...
export async function POST(request: NextResponse) {
  try {
    ...
    if (uploadToDrive) {
      ...
      const driveFolderID = process.env.COI_AI_DRIVE_FOLDER_ID as string
      ...
      const fileName = `COI_AI_${businessName}_${issueDate}_${logId}`;
    ...
    if (logToSheets) {
      ...
      const spreadsheetId = process.env.COI_AI_SPREADSHEET_ID;
      const range = "Sheet1!A2:B";
      const getResponse = await sheets.spreadsheets.values.get({ spreadsheetId, range });
      const rows = getResponse.data.values || [];
      const nextRow = rows.length + 2;
      const values = [[businessName, businessName2, clubName, amount, issueDate, expiryDate, notes, driveFileUrl, submittedBy, currentTime, logId]];
